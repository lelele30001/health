      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="食谱分类" prop="shipufenlei">
            ...
            >
              <el-option
                v-for="(item, index) in shipufenleiLists"
                :label="item"
                :value="item"
              >
              </el-option>
        ...
        <el-col :span="12">
          <el-form-item label="食材" prop="shicai">
            <el-input
              ...
              placeholder="食材"
              type="text"
              :readonly="!isAdd || disabledForm.shicai ? true : false"
            />
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item label="配料" prop="peiliao">
            <el-input
              class="list_inp"
              v-model="form.peiliao"
              placeholder="配料"
              type="text"
              :readonly="!isAdd || disabledForm.peiliao ? true : false"
            />
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item label="适宜人群" prop="shiyirenqun">
            <el-input
              class="list_inp"
              v-model="form.shiyirenqun"
              placeholder="适宜人群"
              type="text"
              :readonly="!isAdd || disabledForm.shiyirenqun ? true : false"
            />
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12">
          <el-form-item label="相关技巧" prop="xiangguanjiqiao">
            <el-input
              class="list_inp"
              v-model="form.xiangguanjiqiao"
              placeholder="相关技巧"
              type="text"
              :readonly="!isAdd || disabledForm.xiangguanjiqiao ? true : false"
            />
          </el-form-item>
        </el-col>
        ...
        <el-col :span="24">
          <el-form-item label="食谱介绍" prop="shipujieshao">
            <editor
              class="list_editor"
              :value="form.shipujieshao"
              placeholder="请输入食谱介绍"
              :readonly="!isAdd || disabledForm.shipujieshao ? true : false"
              @change="(e) => editorChange(e, 'shipujieshao')"
            ></editor>
          </el-form-item>
        </el-col>
      </el-row>
...
const getInfo = () => {
  context
    ...
    .then((res) => {
      ...
      res.data.data.shipujieshao = res.data.data.shipujieshao.replace(
        reg,
        "../../../project/file",
      );
...
const save = () => {
  if (form.value.shiputupian != null) {
    form.value.shiputupian = form.value.shiputupian.replace(
      new RegExp(context?.$config.url, "g"),
      "",
    );
  }
  // 创建食谱对象，将shiputupian的值赋给coverImage
  const recipeData = {
    ...form.value,
    coverImage: form.value.shiputupian,
    title: form.value.shipumingcheng,
    description: form.value.shipujieshao,
    fitnessGoal: form.value.shipufenlei,
    dietaryRestrictions: form.value.shiyirenqun,
    nutritionTag: form.value.xiangguanjiqiao,
    mealScene: form.value.shipufenlei,
    totalCalories: 0,
    status: "已发布",
  };
  ...
  formRef.value.validate((valid) => {
    if (valid) {
      if (crossUserId && crossRefId) {
        recipeData.crossuserid = crossUserId;
        recipeData.crossrefid = crossRefId;
        let params = {
          page: 1,
          limit: 1000,
          crossuserid: recipeData.crossuserid,
          crossrefid: recipeData.crossrefid,
        };
        context
          ?.$http({
            url: `recipe/publish`,
