	...
	<div class="list-page" :style='{}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('shipuxinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.shiputupian && item.shiputupian.substr(0,4)=='http'" @click.stop="preViewClick(item.shiputupian)">
							<el-image class="data_img" :src="item.shiputupian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.shiputupian.split(',')[0])">
							<el-image class="data_img" :src="item.shiputupian?$config.url + item.shiputupian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title1">
								<span>{{item.shipumingcheng}}</span>
							</div>
						</div>
					</div>
				</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/shipuxinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.shipufenlei = categoryList.value[categoryIndex.value]
		}
