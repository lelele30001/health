	...
	<div class="list-page" :style='{}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('shicaixinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
        ...
        <div class="sort-wrapper">
            <el-button class="item click" @click="sortClick('clickNumber')" :class="{active:sortType=='clickNumber'}">
                <el-icon class="icon" v-if="sortType!='clickNumber'"><DCaret /></el-icon>
			...
			<div class="data_box">
				<div class="data_view">
					<div class="data_item" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						<div class="data_img_box" v-if="item.shicaitupian && item.shicaitupian.substr(0,4)=='http'" @click.stop="preViewClick(item.shicaitupian)">
							<el-image class="data_img" :src="item.shicaitupian" fit="cover"></el-image>
						</div>
						<div class="data_img_box" v-else @click.stop="preViewClick($config.url+item.shicaitupian.split(',')[0])">
							<el-image class="data_img" :src="item.shicaitupian?$config.url + item.shicaitupian.split(',')[0]:''"
								fit="cover"></el-image>
						</div>
						<div class="data_content">
							<div class="data_title1">
								<span>{{item.shicaimingcheng}}</span>
							</div>
						</div>
					</div>
				</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/shicaixinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.shicaileixing = categoryList.value[categoryIndex.value]
		}
