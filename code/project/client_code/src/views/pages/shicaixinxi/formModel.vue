			...
			<div class="info_view">
				<div class="title_view">
					<div class="detail_title">
						<span>{{detail.shicaimingcheng}}</span>
					</div>
				...
				<div class="info_item">
					<div class="info_label">食材类型</div>
					<div  class="info_text" >{{detail.shicaileixing}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">食材功效</div>
					<div  class="info_text" >{{detail.shicaigongxiao}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">保存方式</div>
					<div  class="info_text" >{{detail.baocunfangshi}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">食材用途</div>
					<div  class="info_text" >{{detail.shicaiyongtu}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button class="edit_btn" v-if="centerType&&btnAuth('shicaixinxi','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('shicaixinxi','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
		<div class="detail-box">
			<div class="title"><span>食材详情</span></div>
			<div class="content" v-html="detail.shicaixiangqing"></div>
		</div>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
            ...
            bannerList.value = res.data.data.shicaitupian?res.data.data.shicaitupian.split(','):[]
            title.value = res.data.data.shicaimingcheng
	...
	const collectClick = (type) => {
		if (type == 1 && !collectType.value) {
			let params = {
				name: title.value,
				picture: bannerList.value[0],
				inteltype: detail.value.shicaileixing,
				refid: detail.value.id,
				type: type,
				tablename: tableName,
				userid: context?.$toolUtil.storageGet('userid')
